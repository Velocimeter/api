...
from app.settings import LOGGER, CACHE, VE_ADDRESS, VOTER_ADDRESS, REWARDS_DIST_ADDRESS
...
class VeNFT(Model):
    ...
    def from_chain(cls, address):
        ...
        LOGGER.debug(
            "Fetched data for %s %ss in %s.", len(token_ids), cls.__name__, tdelta
        )
    ...
    def from_chain_calls(cls, account_address, token_id, data):
        ...
        if data["lock_ends_at"] != 0:
            data["lock_ends_at"] = datetime.utcfromtimestamp(data["lock_ends_at"])
        else:
    ...
    def _fetch_token_ids(cls, address):
        ...
        tokens_count = Call(VE_ADDRESS, ["balanceOf(address)(uint256)", address])()
        ...
        for idx in range(0, tokens_count):
            call = Call(
                VE_ADDRESS,
                ["tokenOfOwnerByIndex(address,uint256)(uint256)", address, idx],
